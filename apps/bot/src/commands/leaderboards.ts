...
const LeaderboardsCommand: SlashLevel.ICommand = {
	...
	async execute({ client, interaction }) {
		...
		if (userIndex > -1) {
			const user = ranks[userIndex] as IGuildMemberModel;
			rank = client.utils.formatNumber(userIndex + 1);
			xp = client.utils.formatNumber(user.xp);
			level = client.utils.formatNumber(user.level);
			requiredXP = client.utils.formatNumber(
				client.utils.calculateRequiredExp(user.level + 1),
			);
		}
		...
		for (let i = 0; i < top5.length; i++) {
			const guildMemberModel = top5[i] as IGuildMemberModel;
